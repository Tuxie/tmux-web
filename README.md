...
A browser-based frontend for [tmux](https://github.com/tmux/tmux). Attach to your tmux sessions from any modern browser with a fast, faithful terminal emulator, full mouse and keyboard support, and a themable session/window switcher.
...
- **Real terminal emulator in the browser** — [xterm.js](https://xtermjs.org/) with WebGL, unicode-graphemes, web-links, web-fonts, and image addons.
...
- **Server-side session settings** — per-session colours/font/opacity/background hue/etc. persist in `~/.config/tmux-web/sessions.json` and follow you across browsers.
...
- `tmux` installed and on `PATH`. Latest version recommended.
...
```bash
TMUX_WEB_PASSWORD=changeme tmux-web
```
...
tmux-web forwards OSC 52 clipboard sequences and mouse events from fullscreen terminal editors. Some editors need explicit clipboard or mouse settings for plain `y`/`p` and click-drag behavior to use that path. See [`docs/editors.md`](docs/editors.md) for Vim, Neovim, Emacs, Helix, and Kakoune configuration.
...
```bash
# Direct LAN access allowing 192.168.2.4 to connect (local IP is 192.168.2.2)
tmux-web -l 192.168.2.2:4022 -i 192.168.2.4

# Behind nginx-proxy-manager at 10.0.0.5 proxying https://tmux.example.com
tmux-web -l 0.0.0.0:4022 -i 10.0.0.5 -o https://tmux.example.com
```

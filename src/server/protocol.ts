...
const OSC_52_RE = /\x1b\]52;[^;]*;([A-Za-z0-9+/=]*)(?:\x07|\x1b\\)/g;
...
export function processData(data: string, _currentSession: string): ProcessResult {
  ...
  OSC_52_RE.lastIndex = 0;
  while ((match = OSC_52_RE.exec(data)) !== null) {
    const b64 = match[1];
    if (b64 && b64 !== '?') {
  ...
  OSC_52_RE.lastIndex = 0;
  const output = data.replace(OSC_52_RE, '');

  if (titleChanged && detectedSession) {
    messages.push({ session: detectedSession });
  }

  return { output, messages, titleChanged, detectedSession, detectedTitle };
}
